# ADR-006: Layered (Clean) Architecture for @veap/framework
...
`@veap/framework` originally organised everything under a single `src/core` folder plus `src/modules`. A single directory held the IoC container, the event bus, logging, configuration, error handling and the application bootstrap, mixing four different concerns:
...
- **Self-imports:** modules imported their own package (`@veap/framework/settings/models`), hiding layer boundaries.
...
- Entry points (`src/index.ts`, `src/server.ts`) are barrels only; the `@veap/framework` and `@veap/framework/core/server` subpaths map to `dist/index.js` and `dist/server.js` for backwards compatibility.
