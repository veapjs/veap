...
Welcome to the `@veap/core` package. This package is the unified foundation of the Veap framework, providing the plugin system, virtual router, ActiveRecord ORM, authentication with RBAC, dependency injection container, event bus, and the `veap` CLI.

## 1. Technical Memory & Context

Before making architectural modifications, generating features, or resolving issues in `@veap/core`, you **MUST**:
...
- **Clean Architecture Layers (ADR-006)**:
  ...
  - **No Self-Imports**: Never use `@veap/core/...` inside `packages/veap/src/`. Use relative imports (`../../`).
