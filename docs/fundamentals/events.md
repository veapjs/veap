...
eventBus.subscribe("system:not-installed", "installer-redirect", async (event) => {
  console.warn("System is not installed. Database migrations or initial setup required.");
});
```
...
eventBus.subscribe("system:plugin:toggle", "route-cache-invalidator", async (event) => {
  const { pluginId, enabled } = event.payload;
  console.log(`Plugin ${pluginId} state changed to ${enabled ? "enabled" : "disabled"}. Invalidating router cache.`);
