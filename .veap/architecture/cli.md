...
The CLI is bundled with the core package (`@veap/core`, `packages/veap`): the `veap` binary (`packages/veap/bin/veap.js`) runs the compiled entry `dist/infrastructure/cli/index.js` via Node. There is no separate `@veap/cli` package. The CLI is built with `cac` and runs TypeScript sources through `jiti`.
