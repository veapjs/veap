...
describe("ejectPackage CLI", () => {
  ...
  it("ejects a template into templates/ directory when veap.type is template", async () => {
      ...
      JSON.stringify(
        {
          ...
          repository: "https://github.com/veap-sh/custom-template.git",
          veap: {
            type: "template",
          },
        },
    ...
    expect(execSync).toHaveBeenCalledWith(
      `git clone https://github.com/veap-sh/custom-template.git "${expectedDest}"`,
      { stdio: "ignore" },
    );
  ...
  it("ejects a plugin into plugins/ directory when veap.type is plugin", async () => {
      ...
      JSON.stringify(
        {
          ...
          repository: {
            url: "git+https://github.com/veap-sh/shop-plugin.git",
          },
          veap: {
            type: "plugin",
          },
    ...
    expect(execSync).toHaveBeenCalledWith(
      `git clone https://github.com/veap-sh/shop-plugin.git "${expectedDest}"`,
      { stdio: "ignore" },
    );
