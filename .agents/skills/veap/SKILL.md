...
6. **Check public API boundaries** (Import from `@veap/core/...` not internal paths).
...
- **Transactions:** Always use `await transaction(async () => { ... })` from `@veap/core/database` instead of passing `trx` objects around.
